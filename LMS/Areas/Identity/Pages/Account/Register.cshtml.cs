...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        string CreateNewUser( string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role )
        {
            ...
            if (query.Any())
            {
                var maxUid = query.ToArray().Max().ToString();
                int maxNum = int.Parse(maxUid.Substring(1));
                uId = $"u{(maxNum + 1).ToString("D7")}";
            }
            else
            {
                uId = "u0000003";
            ...
            {
                Administrator dbUser = new Administrator();
                dbUser.FName = firstName;
                dbUser.LName = lastName;
                dbUser.Dob = DateOnly.FromDateTime(DOB);
                dbUser.UId = uId;
                db.Administrators.Add(dbUser);
            }
            else if (role == "Professor")
            {
                Professor dbUser = new Professor();
                dbUser.FName = firstName;
                dbUser.LName = lastName;
                dbUser.Dob = DateOnly.FromDateTime(DOB);
                dbUser.UId = uId;
                dbUser.WorksIn = departmentAbbrev;
                db.Professors.Add(dbUser);
            }
            else 
            {
                Student dbUser = new Student();
                dbUser.FName = firstName;
                dbUser.LName = lastName;
                dbUser.Dob = DateOnly.FromDateTime(DOB);
                dbUser.UId = uId;
                dbUser.Major = departmentAbbrev;
                db.Students.Add(dbUser);
            }
            db.SaveChanges();
            return uId;
        }
